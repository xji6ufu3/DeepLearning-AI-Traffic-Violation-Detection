...
class MyDataset(Data.Dataset):
    def __init__(self, image_path, label_path):
        ...
        for image_name, label_name in zip(image_names, label_names):
            ...
            image_label.append((image_name, label_name))
    ...
    def __getitem__(self, item):
        ...
        image = image/255.0  # 正規化输入
        image = torch.Tensor(image)
        image = image.permute(2, 0, 1)  # 将image的維度轉成network輸入的维度（channel, width, height）

        label = cv.imread(label, 0) #灰度圖像
        label = cv.resize(label, (224, 224))
        #分成2類記得註解
        # label = cv.inRange(label, 60 , 70)
        label[label != 0] = 1 
